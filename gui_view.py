...
class TrainerGuiView:
    def __init__(self, window, start_cmd, stop_cmd, save_cmd, log_cmd, discard_cmd, tutorial_cmd):
        self.window = window
        self.cmds = {
            "start": start_cmd,
            "stop": stop_cmd,
    ...
    def _create_layout(self):
        ...
        self.lbl_description = ctk.CTkLabel(self.sidebar, text=opis_tekst, font=self.fonts["stat_label"], text_color="gray", justify="center")
        ...
        ctk.CTkLabel(self.wizard_controls, text="CEL POWTÓRZEŃ:", font=self.fonts["stat_label"], text_color="gray").pack(pady=(10, 0))
        self.target_reps_var = ctk.IntVar(value=10)
        self.slider = ctk.CTkSlider(self.wizard_controls, from_=1, to=20, variable=self.target_reps_var, number_of_steps=19, button_color="#00E676", progress_color="#00E676")
        self.slider.pack(pady=10)
        self.slider_val_label = ctk.CTkLabel(self.wizard_controls, textvariable=self.target_reps_var, font=self.fonts["btn"])
        self.slider_val_label.pack()
        ctk.CTkButton(self.wizard_controls, text="ZALOGUJ SERIĘ", command=self.cmds["log"], font=self.fonts["btn"], fg_color="#2196F3", height=40).pack(pady=(30, 10), padx=40, fill="x")
        ctk.CTkButton(self.wizard_controls, text="ZAKOŃCZ TRENING", command=self.cmds["stop"], font=self.fonts["btn"], fg_color="#FF5252", hover_color="#D32F2F", height=40).pack(pady=10, padx=40, fill="x")
        self.bottom_frame = ctk.CTkFrame(self.sidebar, fg_color="transparent")
        self.bottom_frame.pack(side="bottom", fill="x", pady=20)
        self.theme_switch = ctk.CTkSwitch(self.bottom_frame, text="Tryb Ciemny", command=self.toggle_theme, progress_color="#00E676")
    ...
    def _build_dashboard(self):
        ctk.CTkButton(self.screen_dashboard, text="NOWA SESJA TRENINGOWA", font=self.fonts["title"], command=self.cmds["start"], height=60, fg_color="#00E676", hover_color="#00C853", text_color="black").pack(pady=(40, 10))
        ctk.CTkButton(self.screen_dashboard, text="TUTORIAL", font=self.fonts["btn"], command=self.cmds["tutorial"], height=40, fg_color="#2196F3", hover_color="#1976D2", text_color="white").pack(pady=(0, 40))
        ...
        ctk.CTkLabel(self.screen_dashboard, text="HISTORIA SESJI:", font=self.fonts["header"], text_color="gray").pack(anchor="w", padx=30, pady=(40, 10))
    ...
    def _create_stat_card(self, parent, title, val, color):
        ...
        ctk.CTkLabel(frame, text=title, font=self.fonts["stat_label"], text_color="gray").pack(pady=(15, 0), padx=20, anchor="w")
    ...
    def _build_session(self):
        ...
        self.status_label = ctk.CTkLabel(self.screen_session, text="Gotowy.", font=self.fonts["header"], fg_color=("gray80", "gray20"), corner_radius=10, height=40)
    ...
    def _build_summary(self):
        ...
        ctk.CTkButton(btn_frame, text="ZAPISZ DO HISTORII", command=self.cmds["save"], font=self.fonts["btn"], fg_color="#00E676", hover_color="#00C853", text_color="black", height=50).pack(side="left", expand=True, padx=10)
        ctk.CTkButton(btn_frame, text="ODRZUĆ", command=self.cmds["discard"], font=self.fonts["btn"], fg_color="#FF5252", hover_color="#D32F2F", height=50).pack(side="left", expand=True, padx=10)
    ...
    def show_dashboard(self, summary_data=None):
        ...
        if summary_data:
            ...
            for session in summary_data["sessions_list"][:10]:
                ...
                ctk.CTkLabel(frame, text=f"{session['date']} | {session['name']}", font=self.fonts["btn"]).pack(side="left", padx=20, pady=15)
    ...
    def show_summary(self, session_data):
        ...
        for s in session_data["sets_list"]:
            ...
            frame.pack(fill="x", pady=5)
            ctk.CTkLabel(frame, text=f"Seria {s['set_id']} (Cel: {s['target_reps']})", font=self.fonts["btn"]).pack(side="left", padx=20, pady=15)
    ...
    def update_live_sets(self, sets_list):
        ...
        for s in sets_list:
            ...
            frame.pack(fill="x", pady=5)
            ctk.CTkLabel(frame, text=f"Seria {s['set_id']} (Cel: {s['target_reps']})", font=self.fonts["btn"]).pack(side="left", padx=20, pady=10)
            ctk.CTkLabel(frame, text=f"{s['logged_reps']} powtórzeń", text_color="gray").pack(side="right", padx=20)
